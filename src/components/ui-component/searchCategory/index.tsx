...
interface ISearchSelect {
  ...
  setRelatedId: (id: number, type: 'add' | 'remove') => void;
}

const SearchProduct: FC<ISearchSelect> = ({ label, inputLabel, options, setRelatedId }) => {
  ...
  const cardHandler = (addProduct: IProductInfo) => {
    ...
    setRelatedId(addProduct.id, 'remove');
  };
